...
use coincync::chain::{
    ...
    REORG_UNCONDITIONAL_DEPTH, MESS_EXPONENT_DIVISOR,
};
...
fn tier1_shallow_reorg_accepted_with_more_work() {
    ...
    let result = evaluate_reorg_acceptability(5, 1000, 999);
...
fn tier1_shallow_reorg_rejected_with_less_work() {
    ...
    let result = evaluate_reorg_acceptability(5, 999, 1000);
...
fn tier1_depth_10_is_unconditional() {
    ...
    let result = evaluate_reorg_acceptability(REORG_UNCONDITIONAL_DEPTH, 1001, 1000);
...
fn tier1_equal_work_rejected() {
    ...
    let result = evaluate_reorg_acceptability(5, 1000, 1000);
...
fn tier2_depth_11_requires_more_than_just_more_work() {
    ...
    let result = evaluate_reorg_acceptability(11, 1001, 1000);
...
fn tier2_depth_30_requires_2x_work() {
    ...
    let result = evaluate_reorg_acceptability(30, required, honest);
    assert!(result.is_err(), "Depth 30 with exactly 2x work must be rejected (needs >2x)");

    // Above threshold — accepted
    let result = evaluate_reorg_acceptability(30, required + 1, honest);
...
fn tier2_depth_50_requires_4x_work() {
    ...
    let result = evaluate_reorg_acceptability(50, required, honest);
    assert!(result.is_err(), "Depth 50 with exactly 4x must be rejected");

    let result = evaluate_reorg_acceptability(50, required + 1, honest);
...
fn tier2_depth_70_requires_8x_work() {
    ...
    let result = evaluate_reorg_acceptability(70, required, honest);
    assert!(result.is_err(), "Depth 70 with exactly 8x must be rejected");

    let result = evaluate_reorg_acceptability(70, required + 1, honest);
...
fn tier2_depth_90_requires_16x_work() {
    ...
    let result = evaluate_reorg_acceptability(90, required, honest);
    assert!(result.is_err(), "Depth 90 with exactly 16x must be rejected");

    let result = evaluate_reorg_acceptability(90, required + 1, honest);
...
fn tier2_economics_rental_attack_infeasible() {
    ...
    let result = evaluate_reorg_acceptability(50, attacker_work, honest_work);
    assert!(
        result.is_err(),
    ...
    let result = evaluate_reorg_acceptability(50, attacker_2x, honest_work);
    assert!(
        result.is_err(),
...
fn tier3_depth_101_hard_rejected() {
    ...
    let result = evaluate_reorg_acceptability(max + 1, u128::MAX, 1);
    assert!(
        result.is_err(),
...
fn tier3_depth_max_u64_hard_rejected() {
    let result = evaluate_reorg_acceptability(u64::MAX, u128::MAX, 1);
...
fn edge_zero_depth_accepted() {
    ...
    let result = evaluate_reorg_acceptability(0, 1001, 1000);
...
fn edge_honest_work_zero() {
    ...
    let result = evaluate_reorg_acceptability(5, 1, 0);
...
fn edge_overflow_resistance() {
    ...
    let result = evaluate_reorg_acceptability(50, u128::MAX, u128::MAX / 2);
