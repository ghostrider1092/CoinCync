...
fn tier1_shallow_reorg_accepted_with_more_work() {
    ...
    assert!(
        result.is_ok(),
        "Shallow reorg with more work must be accepted: {:?}",
        result
    );
}
...
fn tier1_shallow_reorg_rejected_with_less_work() {
    ...
    assert!(
        result.is_err(),
        "Shallow reorg with less work must be rejected"
    );
}
...
fn tier1_depth_10_is_unconditional() {
    ...
    let result = evaluate_reorg_acceptability(
        REORG_UNCONDITIONAL_DEPTH,
        1001,
        1000,
        POST_BOOTSTRAP,
        max_depth(),
    ...
    assert!(
        result.is_ok(),
        "Depth {} must still be unconditional",
        REORG_UNCONDITIONAL_DEPTH
    );
}
...
fn tier2_depth_11_requires_more_than_just_more_work() {
    ...
    assert!(
        result.is_ok(),
        "Depth 11 with slightly more work should pass (multiplier=1)"
    );
}
...
fn tier2_depth_30_requires_2x_work() {
    ...
    assert!(
        result.is_err(),
        "Depth 30 with exactly 2x work must be rejected (needs >2x)"
    );

    // Above threshold — accepted
    let result =
        evaluate_reorg_acceptability(30, required + 1, honest, POST_BOOTSTRAP, max_depth());
...
fn tier2_depth_50_requires_4x_work() {
    ...
    let result =
        evaluate_reorg_acceptability(50, required + 1, honest, POST_BOOTSTRAP, max_depth());
...
fn tier2_depth_70_requires_8x_work() {
    ...
    let result =
        evaluate_reorg_acceptability(70, required + 1, honest, POST_BOOTSTRAP, max_depth());
...
fn tier2_depth_90_requires_16x_work() {
    ...
    assert!(
        result.is_err(),
        "Depth 90 with exactly 16x must be rejected"
    );

    let result =
        evaluate_reorg_acceptability(90, required + 1, honest, POST_BOOTSTRAP, max_depth());
...
fn tier2_economics_rental_attack_infeasible() {
    ...
    let result =
        evaluate_reorg_acceptability(50, attacker_work, honest_work, POST_BOOTSTRAP, max_depth());
    assert!(
        result.is_err(),
    ...
    let result =
        evaluate_reorg_acceptability(50, attacker_2x, honest_work, POST_BOOTSTRAP, max_depth());
    assert!(
        result.is_err(),
...
fn tier3_beyond_max_hard_rejected() {
    ...
    assert!(
        result.is_err(),
        "Depth {} (> max {}) must be hard-rejected",
        max + 1,
        max
    );
}
...
fn tier3_depth_max_u64_hard_rejected() {
    ...
    assert!(
        result.is_err(),
        "Depth u64::MAX must be hard-rejected without overflow"
    );
}
...
fn edge_honest_work_zero() {
    ...
    assert!(
        result.is_ok(),
        "Fork with work=1 vs honest=0 must be accepted"
    );
}
...
fn edge_overflow_resistance() {
    ...
    let result =
        evaluate_reorg_acceptability(50, u128::MAX, u128::MAX / 2, POST_BOOTSTRAP, max_depth());
...
fn constants_are_sane() {
    assert_eq!(
        ...
        "Unconditional depth must be 10"
    );
