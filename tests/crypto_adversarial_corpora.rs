...
use coincync::crypto::{
    SecretScalar, PublicPoint, EcCommitment, KeyImage as CryptoKeyImage, hash_to_point,
...
fn range_proof_at_u64_max() {
    ...
    let proof = create_range_proof_for_height(
        Amount::from_atomic(value), &blinding, &mut OsRng, 0
    );
    match proof {
        Ok(p) => {
            assert!(verify_range_proof_dispatch(&commitment, &p, 0),
...
fn range_proof_at_power_of_2_boundaries() {
    ...
    for exp in [1u32, 2, 4, 8, 16, 31, 32, 48, 63] {
        ...
        assert!(verify_range_proof_dispatch(&commitment, &proof, 0),
            "range proof for 2^{} = {} must verify", exp, value);
    }
}
...
fn range_proof_at_power_of_2_minus_1() {
    for exp in [8u32, 16, 32, 48, 63] {
        ...
        assert!(verify_range_proof_dispatch(&commitment, &proof, 0),
            "range proof for 2^{}-1 = {} must verify", exp, value);
    }
}
...
fn commitment_add_with_identity_is_self() {
    ...
    assert_eq!(sum.to_bytes(), c.to_bytes(),
        "C + identity should equal C");
}
...
fn keyimage_from_group_order_minus_1() {
    ...
    for b in bytes.iter_mut() {
        if borrow {
            if *b == 0 { *b = 0xFF; } else { *b -= 1; borrow = false; }
        }
    }
    let s = SecretScalar::from_bytes(bytes);
    let ki = CryptoKeyImage::from_secret(&s);
    assert_ne!(ki.to_bytes(), [0u8; 32],
...
fn all_ones_scalar() {
    ...
    let p = s.to_public();
    assert_ne!(p.to_bytes(), [0u8; 32], "all-0x01 scalar should not be identity");
}
...
fn single_output_amounts_corpus() {
    ...
    for value in test_values {
        ...
        assert!(verify_range_proof_dispatch(&commitment, &proof, 0),
            "range proof for value {} must verify", value);
    }
}
