...
async fn start_test_server_with_peer_fixture_loopback(
    ...
    let mut node_cfg = NodeConfig::default();
    node_cfg.data_dir =
        std::env::temp_dir().join(format!("coincync-rpc-peer-fixture-loopback-{}", port));
    let p2p = Arc::new(P2PNode::new(
        node_cfg,
        shared_chain.clone(),
        shared_mempool.clone(),
...
async fn rpc_get_info_returns_result() {
    ...
    assert!(
        ...
        "get_info must return result: {}",
        resp
    );
    let result = &resp["result"];
    assert!(result.get("height").is_some(), "get_info must have height");
    assert!(
        result.get("network").is_some(),
        "get_info must have network"
    );
    assert!(
        result.get("is_synced").is_some(),
        "get_info must have is_synced"
    );
}
...
async fn rpc_get_blockchain_info_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_blockchain_info must return result: {}",
        resp
    );
}
...
async fn rpc_get_mempool_info_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_mempool_info must return result: {}",
        resp
    );
    let result = &resp["result"];
...
async fn rpc_get_supply_info_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_supply_info must return result: {}",
        resp
    );
}
...
async fn rpc_get_privacy_stats_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_privacy_stats must return result: {}",
        resp
    );
}
...
async fn rpc_get_network_info_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_network_info must return result: {}",
        resp
    );
}
...
async fn rpc_get_sync_status_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_sync_status must return result: {}",
        resp
    );
}
...
async fn rpc_get_anonymity_set_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_anonymity_set must return result: {}",
        resp
    );
}
...
async fn rpc_get_chain_events_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_chain_events must return result: {}",
        resp
    );
}
...
async fn rpc_get_mining_live_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_mining_live must return result: {}",
        resp
    );
}
...
async fn rpc_get_block_template_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_block_template must return result: {}",
        resp
    );
}
...
async fn rpc_get_asset_info_returns_error() {
    ...
    assert!(
        ...
        "get_asset_info must return error in 1.0 (no asset layer): {}",
        resp
    );
}
...
async fn rpc_get_block_by_height_valid() {
    ...
    assert!(
        ...
        "get_block_by_height must return result or error, not crash: {}",
        resp
    );
}
...
async fn rpc_get_block_by_height_future() {
    ...
    assert!(
        resp.get("error").is_some(),
        "nonexistent height must return error: {}",
        resp
    );
}
...
async fn rpc_get_block_by_hash_missing() {
    ...
    assert!(
        resp.get("error").is_some(),
        "nonexistent block hash must return error: {}",
        resp
    );
}
...
async fn rpc_get_transaction_missing() {
    ...
    assert!(
        resp.get("error").is_some(),
        "nonexistent tx must return error: {}",
        resp
    );
}
...
async fn rpc_get_block_range_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "get_block_range must return result: {}",
        resp
    );
}
...
async fn rpc_is_nullifier_spent_returns_result() {
    ...
    assert!(
        resp.get("result").is_some(),
        "is_nullifier_spent must return result: {}",
        resp
    );
}
...
async fn rpc_get_block_by_height_string_param() {
    ...
    assert!(
        resp.get("error").is_some(),
        "string param for height must return error: {}",
        resp
    );
}
...
async fn rpc_get_block_by_height_negative() {
    ...
    assert!(
        resp.get("error").is_some(),
        "negative height must return error: {}",
        resp
    );
}
...
async fn rpc_submit_block_garbage() {
    ...
    assert!(
        resp.get("error").is_some(),
        "garbage block must return error: {}",
        resp
    );
}
...
async fn rpc_get_block_by_height_no_params() {
    ...
    assert!(
        resp.get("error").is_some(),
        "missing params must return error: {}",
        resp
    );
}
...
async fn rpc_submit_block_empty_hex() {
    ...
    assert!(
        resp.get("error").is_some(),
        "empty hex block must return error: {}",
        resp
    );
}
...
async fn rpc_get_transaction_invalid_hex() {
    ...
    assert!(
        resp.get("error").is_some(),
        "invalid hex tx hash must return error: {}",
        resp
    );
}
...
async fn rpc_get_supply_info_has_fields() {
    ...
    assert!(
        ...
        "supply_info must have at least one supply field: {}",
        r
    );
}
...
async fn rpc_get_privacy_stats_has_fields() {
    ...
    assert!(
        ...
        "privacy_stats must have at least one field: {}",
        r
    );
}
...
async fn rpc_info_reports_runtime_hardening_posture() {
    ...
    let r = &resp["result"];
    assert_eq!(
        r.get("rpc_auth_enabled").and_then(|v| v.as_bool()),
        Some(false)
    );
    assert_eq!(
        r.get("metadata_minimized").and_then(|v| v.as_bool()),
...
async fn rpc_public_bind_rejects_missing_bearer_for_get_peers() {
    ...
    assert_eq!(
        resp["error"]["code"], 401,
        "public bind should reject missing bearer auth"
    );
}
...
async fn rpc_public_bind_get_peers_response_shape_is_privacy_safe() {
    ...
    assert!(
        r.get("peers").and_then(|v| v.as_array()).is_some(),
    ...
    if let Some(peers) = r.get("peers").and_then(|v| v.as_array()) {
        for p in peers {
            assert_eq!(
                p.get("metadata_minimized").and_then(|v| v.as_bool()),
                Some(true)
            );
            assert_eq!(p.get("addr").and_then(|v| v.as_str()), Some("[redacted]"));
            assert_eq!(
                p.get("user_agent").and_then(|v| v.as_str()),
                Some("[redacted]")
            );
...
async fn rpc_public_bind_redacts_real_peer_fixture_fields() {
    ...
    assert!(
        !peers.is_empty(),
        "fixture should produce at least one peer"
    );
    let p = &peers[0];
...
async fn rpc_loopback_exposes_peer_fixture_fields_when_not_minimized() {
    ...
    let (url, _server) =
        start_test_server_with_peer_fixture_loopback(19148, config, peer.clone()).await;
    let resp = rpc_call(&url, "get_peers", json!([])).await;
    let peers = resp["result"]["peers"].as_array().expect("peers array");
    assert!(
        !peers.is_empty(),
...
async fn rpc_info_reports_stratum_posture_hardened_when_not_public() {
    ...
    let r = &resp["result"];
    assert_eq!(
        r.get("stratum_public_bind_requested")
            .and_then(|v| v.as_bool()),
        Some(false)
    );
    assert_eq!(
        r.get("stratum_transport_hardened")
            .and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
async fn rpc_info_reports_stratum_posture_unhardened_when_public_without_tls() {
    ...
    assert_eq!(
        r.get("stratum_public_bind_ack").and_then(|v| v.as_bool()),
        Some(true)
    );
    assert_eq!(
        r.get("stratum_native_tls_enabled")
            .and_then(|v| v.as_bool()),
        Some(false)
    );
    assert_eq!(
        r.get("stratum_tls_proxy_ack").and_then(|v| v.as_bool()),
        Some(false)
    );
    assert_eq!(
        r.get("stratum_transport_hardened")
