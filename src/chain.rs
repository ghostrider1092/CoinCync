...
impl ChainStats {
    /// Cumulative emitted supply in atomic units, saturated to `u64` for
    /// JSON-RPC display.
    ///
    /// The accumulator is `u128` (see `total_supply`), but `serde_json` without
    /// `arbitrary_precision` can't represent a JSON number above `u64::MAX`, and
    /// existing clients parse the supply fields as numbers. The cap only bites
    /// above ~18.4M CYNC of cumulative emission (far future); the chain keeps the
    /// exact `u128` supply regardless — this is a display-only clamp.
    #[inline]
    pub fn total_supply_u64_saturating(&self) -> u64 {
        self.total_supply.min(u64::MAX as u128) as u64
    }
}
...
fn inner_stats_for_log(inner: &RwLock<BlockchainInner>) -> (u128, u64) {
    ...
    // supply saturated to u64 for the tracing field (tracing has no u128
    // Value impl); the accumulator itself is u128.
    (guard.stats.total_difficulty, guard.stats.total_supply.min(u64::MAX as u128) as u64)
}
...
mod tests {
    ...
    fn total_supply_accumulator_is_u128_and_survives_the_old_u64_ceiling() {
        ...
        // JSON-RPC display saturates (serde_json can't hold a Number > u64::MAX)
        // rather than wrapping; the chain keeps the exact u128 value.
        assert_eq!(stats.total_supply_u64_saturating(), u64::MAX);
    }
