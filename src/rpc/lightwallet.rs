...
impl LightWalletServer {
    ...
    pub fn chain_info(&self) -> WalletChainInfo {
        ...
        WalletChainInfo {
            ...
            // v1.0.12 fix: the ring size advertised to external light
            // wallets must reflect what the validator will require for
            // a tx mined into the NEXT block (chain tip + 1), not a
            // hardcoded bootstrap value. Light wallets that trust this
            // field to build their tx must see the height-aware ring
            // size, otherwise they build ring-11 txs at h≥5000 (the
            // v1.0.12 ramp activation) and get InvalidRingSize rejection.
            //
            // Wraps over u64::MAX would never happen at tip levels but
            // saturating_add is defensive — a tip-at-u64::MAX node has
            // bigger problems than ring size selection.
