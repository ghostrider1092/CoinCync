...
//!
//! - **§1 `generate_self_signed_cert`** — INVARIANT: generates an ECDSA P-256
//!   self-signed cert + key only when absent, with a valid future expiry. THREAT:
//!   an unexpected regeneration silently breaks a client's pinned fingerprint.
//!   TESTS: `test_generate_self_signed_cert`, `test_cert_expiry`.
//! - **§2 `set_restrictive_permissions`** — INVARIANT: the private-key file is
//!   written owner-only. THREAT: a world-readable RPC key leaks the server's TLS
//!   identity. TESTS: `test_set_restrictive_permissions_owner_only`.
//! - **§3 `load_pem_cert_and_key`** — INVARIANT: parses PKCS8/PKCS1/SEC1 key tags
//!   and rejects empty or missing PEM blocks. THREAT: silently loading no key or a
//!   garbage key → server comes up with broken TLS. TESTS:
//!   `test_load_pem_rejects_empty_and_missing_blocks`,
//!   `test_load_pem_parses_pkcs8_pkcs1_sec1_key_tags`.
//! - **§4 `load_server_tls_config`** — INVARIANT: builds the rustls server config
//!   from the loaded cert/key pair. THREAT: a misconfigured server config serves
//!   without the intended cert. TESTS: (gap — exercised only indirectly via the
//!   generate + load_pem tests; no standalone server-config assertion).
//! - **§5 `load_client_tls_config` / fingerprint pinning** — INVARIANT: a pinned
//!   fingerprint is enforced and a mismatch is rejected, an invalid fingerprint
//!   string errors, and the no-pin path uses default verification. THREAT: a MITM
//!   cert accepted because pinning is bypassed or mis-compared. TESTS:
//!   `test_load_client_config_no_fingerprint`, `test_load_client_config_with_fingerprint`,
//!   `test_load_client_config_invalid_fingerprint`, `test_fingerprint_pinning_verification`,
//!   `test_blake3_verifier_rejects_fingerprint_mismatch`.
//! - **§6 `cert_fingerprint`** — INVARIANT: computes the BLAKE3 certificate
//!   fingerprint used for pin comparison. THREAT: a wrong digest breaks the pin
//!   match on both ends. TESTS: `test_fingerprint_pinning_verification`,
//!   `test_blake3_verifier_rejects_fingerprint_mismatch`.
...
mod tests {
    ...
    #[test]
    fn test_blake3_verifier_rejects_fingerprint_mismatch() {
        use rustls::client::danger::ServerCertVerifier;
        ensure_crypto_provider();
        let dir = std::env::temp_dir().join("coincync_test_tls_mismatch");
        let _ = std::fs::remove_dir_all(&dir);

        let (cert_path, key_path) = generate_self_signed_cert(&dir, "coincync-mismatch").unwrap();
        let (certs, _key) = load_pem_cert_and_key(&cert_path, &key_path).unwrap();
        let end_entity = &certs[0];
        let server_name = rustls::pki_types::ServerName::try_from("localhost").unwrap();

        // Pin to a fingerprint that does not match the presented cert → rejected.
        let verifier = Blake3CertVerifier {
        ...
        // Pin to the correct fingerprint → accepted.
        let actual = blake3::hash(end_entity.as_ref());
        let verifier_ok = Blake3CertVerifier {
            expected: Some(*actual.as_bytes()),
        };
        let res_ok = verifier_ok.verify_server_cert(
            end_entity,
            &[],
            &server_name,
            &[],
    ...
    #[test]
    fn test_set_restrictive_permissions_owner_only() {
        let dir = std::env::temp_dir().join("coincync_test_tls_perms");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();

        let key_file = dir.join("secret.key");
        std::fs::write(&key_file, b"secret material").unwrap();
        set_restrictive_permissions(&key_file).unwrap();
        ...
        // Silence unused warning on platforms without a cfg branch above.
        let _ = &meta;
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn test_load_pem_rejects_empty_and_missing_blocks() {
        ensure_crypto_provider();
        let dir = std::env::temp_dir().join("coincync_test_tls_pem_reject");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();

        let (real_cert, real_key) = generate_self_signed_cert(&dir, "coincync-pem").unwrap();

        // Empty cert PEM → no certificates found.
        let empty_cert = dir.join("empty_cert.pem");
        std::fs::write(&empty_cert, b"").unwrap();
        assert!(load_pem_cert_and_key(&empty_cert, &real_key).is_err());

        // Valid cert but a key file with no key block → no private key found.
        let no_key = dir.join("no_key.pem");
        std::fs::write(&no_key, b"").unwrap();
        assert!(load_pem_cert_and_key(&real_cert, &no_key).is_err());

        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn test_load_pem_parses_pkcs8_pkcs1_sec1_key_tags() {
        ensure_crypto_provider();
        let dir = std::env::temp_dir().join("coincync_test_tls_keytags");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();

        let (real_cert, _) = generate_self_signed_cert(&dir, "coincync-keytags").unwrap();

        // The loader dispatches purely on the PEM tag; the key bytes are wrapped
        // without validation, so arbitrary contents exercise each variant.
        let cases: [(&str, &str); 3] = [
            ("PRIVATE KEY", "pkcs8.pem"),
            ("RSA PRIVATE KEY", "pkcs1.pem"),
            ("EC PRIVATE KEY", "sec1.pem"),
        ];
        for (tag, file) in cases {
            let key_pem = pem::encode(&pem::Pem::new(tag, vec![0u8; 32]));
            let key_file = dir.join(file);
            std::fs::write(&key_file, key_pem).unwrap();

            let (_certs, key) = load_pem_cert_and_key(&real_cert, &key_file).unwrap();
            match tag {
                "PRIVATE KEY" => {
