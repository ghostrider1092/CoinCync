...
//! THREAT it defends, and the TESTS that prove it. (Integration tests live in
//! `tests/rest_endpoints_extra.rs` and `tests/rest_rate_limit.rs`; many are
//! `#[tokio::test]` handlers.)
//!
//! - **§1 `rpc_proxy` / `jsonrpc_call` (allowlist + error mapping)** — INVARIANT:
//!   only allowlisted read-only RPC methods are forwarded; sensitive methods are
//!   blocked; backend error codes map to HTTP status; an unreachable backend
//!   returns a sanitized 502; oversize bodies 413 and invalid JSON 400; a
//!   configured bearer is forwarded. THREAT: an attacker reaches a write/operator
//!   method or leaks backend internals through the proxy. TESTS:
//!   `test_rpc_allowlist_has_explorer_methods`, `test_rpc_allowlist_blocks_sensitive`,
//!   `jsonrpc_call_maps_backend_error_codes_to_http_status`,
//!   `jsonrpc_call_backend_unreachable_returns_502_sanitized`,
//!   `jsonrpc_call_forwards_bearer_when_configured`,
//!   `rest_rpc_proxy_rejects_disallowed_method_403`,
//!   `rest_rpc_proxy_rejects_oversize_body_413`, `rest_rpc_proxy_rejects_invalid_json_400`.
//! - **§2 privacy field stripping** — INVARIANT: forwarded `get_info`/status/network
//!   responses strip privacy/operator fields and ring correlators. THREAT: the
//!   public explorer leaks operator-only or deanonymizing data. TESTS:
//!   `rest_rpc_proxy_get_info_strips_privacy_and_operator_fields`,
//!   `rest_status_omits_ring_correlators`, `rest_network_omits_effective_ring_size`.
//! - **§3 rate limiting (`enforce_fixed_window_limit` / `enforce_ip_fixed_window_limit`
//!   / `client_ip_from_headers` / `trust_proxy_headers`)** — INVARIANT: per-window
//!   and per-IP fixed-window limits return 429 under burst, the client IP is taken
//!   from forwarded headers ONLY when proxy trust is enabled, and stale entries
//!   prune at cap. THREAT: DoS, or IP spoofing via a forged `X-Forwarded-For` to
//!   evade the limit. TESTS: `rest_rpc_proxy_rate_limit_returns_429_under_burst`,
//!   `rest_stats_rate_limit_returns_429_under_burst`,
//!   `rest_recent_blocks_paginates_and_rate_limits`,
//!   `rest_submit_transaction_rate_limits_at_5_per_sec`,
//!   `client_ip_from_headers_ignores_forwarded_headers_without_trust_env`,
//!   `enforce_ip_fixed_window_limit_prunes_stale_entries_at_cap`.
//! - **§4 input validation (`validate_hex_hash` / pagination / `search`)** —
//!   INVARIANT: hex-hash params are validated, pagination clamps to bounds, and
//!   search rejects empty/overlong/no-match queries. THREAT: injection or resource
//!   abuse through malformed path/query params. TESTS: `test_validate_hex_hash_valid`,
//!   `test_validate_hex_hash_too_short`, `test_validate_hex_hash_non_hex`,
//!   `test_pagination_defaults`, `test_pagination_clamp`,
//!   `search_rejects_empty_and_overlong_queries`,
//!   `rest_search_guards_empty_overlong_and_no_match`, `rest_block_by_hash_invalid_returns_400`.
//! - **§5 supply endpoints (`get_supply` / `parse_total_emitted_atomic` /
//!   `format_atomic_supply_micro`)** — INVARIANT: circulating supply parses without
//!   precision loss and formats correctly above `u64`, rejecting lossy or invalid
//!   RPC values. THREAT: a supply above `u64` truncates → wrong economic display.
//!   TESTS: `circulating_formatter_preserves_supply_above_u64`,
//!   `circulating_parser_rejects_lossy_or_invalid_rpc_values`,
//!   `rest_supply_endpoints_return_happy_payloads`.
//! - **§6 block / tx / mempool read handlers** — INVARIANT: block-by-hash/height,
//!   block-transactions, transaction, recent-blocks, and mempool endpoints return
//!   the expected shapes and 400/404 on bad or missing input. THREAT: a malformed
//!   lookup 500s or leaks. TESTS: `param_routes_match_real_values_not_404`,
//!   `rest_block_by_height_happy_and_non_numeric`,
//!   `rest_block_transactions_returns_list_shape`,
//!   `rest_transaction_invalid_400_and_missing_404`, `rest_mempool_and_stats_happy`.
//! - **§7 `submit_transaction`** — INVARIANT: rejects non-hex and oversize payloads
//!   with 400 and rate-limits submissions. THREAT: malformed or flooded tx
//!   submissions abuse the write path. TESTS:
//!   `rest_submit_transaction_rejects_non_hex_and_oversize_400`,
//!   `rest_submit_transaction_rate_limits_at_5_per_sec`.
//! - **§8 health endpoints (`health` / `health_live` / `health_ready`)** —
//!   INVARIANT: liveness returns 200 regardless of backend; readiness returns 503
//!   when the backend is unreachable. THREAT: a load balancer routes traffic to a
//!   node whose backend is dead. TESTS: `test_health_endpoint`,
//!   `test_health_live_returns_200_regardless_of_backend`,
//!   `test_health_ready_returns_503_when_backend_unreachable`.
//! - **§9 websocket cap (`ws_per_ip_check_and_increment` / `ws_handler`)** —
//!   INVARIANT: per-IP WS connections count up to a cap then reject, and decrement
//!   on drop; ping/pong works. THREAT: WS connection exhaustion from a single IP.
//!   TESTS: `ws_per_ip_guard_counts_up_to_cap_then_rejects_and_decrements`,
//!   `rest_ws_ping_pong_and_per_ip_cap`.
//! - **§10 CORS / assets-gone / emission / events / anonymity** — INVARIANT: CORS
//!   allows known origins and omits unknown; removed asset endpoints return 410
//!   Gone; emission/events/anonymity return valid payloads. THREAT: a stale asset
//!   route or permissive CORS leaks data cross-origin. TESTS:
//!   `rest_cors_allows_known_origin_and_omits_unknown`,
//!   `rest_asset_endpoints_return_410_gone`, `rest_emission_returns_curve_payload`,
//!   `rest_events_returns_happy_payload`, `rest_anonymity_returns_happy_payload`.
...
mod tests {
    ...
    // ─── jsonrpc_call: backend error-code → HTTP status mapping ──────────
    //
    // The mapping logic lives inside `jsonrpc_call` and only fires once a
    // backend has answered with a JSON-RPC `error` object. To exercise it
    // without spinning the full jsonrpsee node + rest router harness we
    // stand up a throwaway one-shot TCP responder that replies with a
    // canned HTTP/1.1 + JSON-RPC body, then call `jsonrpc_call` against it.
    // This keeps the test in the fast in-crate unit binary.
    fn spawn_oneshot_json_backend(
        ...
        let handle = std::thread::spawn(move || {
            ...
            stream
                ...
                .ok();
            // Read up to the end of the request headers (enough to capture the
            // Authorization header for the forwarding test; the body isn't
    ...
    #[tokio::test]
    async fn jsonrpc_call_maps_backend_error_codes_to_http_status() {
        let cases: [(&'static str, StatusCode); 3] = [
            (
                "{\"jsonrpc\":\"2.0\",\"id\":1,\"error\":{\"code\":-5,\"message\":\"not found\"}}",
                StatusCode::NOT_FOUND,
            ),
            (
        ...
        for (body, want) in cases {
            let (addr, handle) = spawn_oneshot_json_backend(body);
            let state = RestState {
                jsonrpc_addr: addr,
                client: reqwest::Client::new(),
                rpc_bearer: None,
            };
            let err = jsonrpc_call(&state, "get_x", Value::Array(vec![]))
                .await
                .unwrap_err();
            assert_eq!(err.0, want, "error body {body} should map to {want}");
            let _ = handle.join();
        }
    }

    #[tokio::test]
    async fn jsonrpc_call_backend_unreachable_returns_502_sanitized() {
        // Port 1 is not listening → the backend request fails at the
        // transport layer. The caller must see a generic 502 with NO
        // internal error text leaked.
        let state = RestState {
            jsonrpc_addr: "127.0.0.1:1".parse().unwrap(),
            client: reqwest::Client::builder()
        ...
        let err = jsonrpc_call(&state, "get_info", Value::Array(vec![]))
            .await
            .unwrap_err();
        ...
        assert_eq!(
            ...
            "backend failure must be sanitized to a generic message"
        );
    }

    #[tokio::test]
    async fn jsonrpc_call_forwards_bearer_when_configured() {
        let (addr, handle) =
            spawn_oneshot_json_backend("{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{}}");
        let state = RestState {
            jsonrpc_addr: addr,
            client: reqwest::Client::new(),
            rpc_bearer: Some("secret-key".to_string()),
        };
        let _ = jsonrpc_call(&state, "get_info", Value::Array(vec![])).await;
        let req = handle.join().unwrap().to_lowercase();
        assert!(
            req.contains("authorization: bearer secret-key"),
            "bearer token must be forwarded to the backend; got request:\n{req}"
        );
    }

    // ─── client_ip_from_headers: XFF trust gating ───────────────────────
    #[test]
    fn client_ip_from_headers_ignores_forwarded_headers_without_trust_env() {
        // With COINCYNC_TRUST_PROXY_HEADERS unset (the default in the test
        // environment) a client cannot spoof X-Forwarded-For / X-Real-IP to
        // escape the shared per-IP bucket — every caller collapses to
        // "unknown". (The trusted path flips a process-global OnceLock, so it
        // is not deterministically togglable per-test; see the extra route
        // suite / deferred notes.)
    ...
    // ─── enforce_ip_fixed_window_limit: opportunistic prune ─────────────
    #[test]
    fn enforce_ip_fixed_window_limit_prunes_stale_entries_at_cap() {
        let table: parking_lot::Mutex<HashMap<String, (u64, u32)>> =
            parking_lot::Mutex::new(HashMap::new());
        let stale_ts = now_unix_secs().saturating_sub(3600);
        {
            let mut g = table.lock();
            for i in 0..IP_WINDOW_MAX_ENTRIES {
                g.insert(format!("stale-{i}"), (stale_ts, 1u32));
            }
            assert_eq!(g.len(), IP_WINDOW_MAX_ENTRIES);
        }
        // Next call for a fresh IP: len >= cap triggers the prune, which drops
        // every stale entry, then inserts the one fresh entry.
        enforce_ip_fixed_window_limit(&table, "fresh-ip", 100).unwrap();
        let g = table.lock();
        assert_eq!(
            g.len(),
            1,
            "stale entries older than IP_WINDOW_STALE_SECS must be pruned at the cap"
        );
        assert!(g.contains_key("fresh-ip"));
    }

    // ─── websocket per-IP guard (the counter WsConnGuard::drop decrements) ─
    #[test]
    fn ws_per_ip_guard_counts_up_to_cap_then_rejects_and_decrements() {
        // Unique key so this test doesn't race the process-global map with
        // any live-WS route test sharing the "unknown" bucket.
        let ip = "ws-unit-fixture-ip";
        for _ in 0..MAX_WS_PER_IP {
            assert!(ws_per_ip_check_and_increment(ip));
        }
        assert!(
            !ws_per_ip_check_and_increment(ip),
            "connection beyond MAX_WS_PER_IP must be rejected"
        );
        // A drop frees a slot — this is exactly what WsConnGuard::drop does.
        ws_per_ip_decrement(ip);
        assert!(
            ws_per_ip_check_and_increment(ip),
            "after a decrement a new connection from the same IP is admitted"
        );
        // Clean up the global map entry.
        for _ in 0..MAX_WS_PER_IP {
            ws_per_ip_decrement(ip);
        }
    }

    // ─── search: length / empty guards (reject before any backend call) ──
    #[tokio::test]
    async fn search_rejects_empty_and_overlong_queries() {
        let state = RestState {
            jsonrpc_addr: "127.0.0.1:1".parse().unwrap(),
            client: reqwest::Client::new(),
            rpc_bearer: None,
        ...
        // Empty query.
        let err = search(State(state.clone()), Query(SearchQuery { q: String::new() }))
            .await
            .unwrap_err();
        ...
        let err = search(
            State(state.clone()),
            Query(SearchQuery {
                q: "a".repeat(MAX_SEARCH_QUERY_LEN + 1),
            }),
        )
        .await
        .unwrap_err();
