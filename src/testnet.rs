...
mod tests {
    ...
    fn test_checkpoints_populated() {
        ...
        for w in heights.windows(2) {
            assert!(
                w[0] < w[1],
                "checkpoint heights not strictly increasing: {} >= {}",
                w[0],
                w[1]
            );
        }
        ...
        for (h, hex_str) in TESTNET_CHECKPOINT_LIST {
            assert_eq!(
                hex_str.len(),
                64,
                "checkpoint at h={} has wrong hex length",
                h
            );
            for c in hex_str.chars() {
                assert!(
                    c.is_ascii_hexdigit(),
                    "checkpoint h={} has non-hex char {:?}",
                    h,
                    c
                );
            }
    ...
    fn test_genesis_checkpoint() {
        ...
        assert_eq!(
            verify_hardcoded_checkpoint(0, &Hash::from_bytes([1u8; 32])),
            Some(false)
        );
    }
}
