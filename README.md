...
Full walkthrough: docs/src/getting-started/build.md.
...
Full per-feature detail (with file:line citations + status): docs/PRIVACY_FEATURES.md.

¹ For the first 10,000 blocks of any network, ring size is 11 (the constitutional BOOTSTRAP_MIN_RING_SIZE — a young chain doesn't have enough on-chain outputs to form a 16-member anonymity set without decoy reuse). Snaps to Ring-16 at height 10,000. scripts/verify-privacy.ps1 is height-aware.
...
Each multi-phase crate's tests/ directory composes every layer in a focused integration test — see tests/README.md for the catalog.

Documentation
Getting started
Consensus specification
Privacy model
Privacy features (per-feature detail)
Node operations
API reference
Constitution — the 19-article + 15-rights compile-time-enforced posture
CIP register — 12 CIPs covering atomic swap, reorg defense, rolling finality, FROST, hard-fork activation policy
Blockchain roadmap — forward-looking, organized by release phase
Operational runbooks — incident response, status page design, reproducible builds, continuous fuzzing, DNS failover, checkpoint procedure
Manifesto
