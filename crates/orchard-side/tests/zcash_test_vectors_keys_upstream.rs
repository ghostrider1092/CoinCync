//! Zcash NU5 Orchard key-component test vectors.
//!
//! **Source:** verbatim copy of `src/test_vectors/keys.rs` from the
//! reference [`orchard` 0.12.0 crate], itself sourced from
//! <https://github.com/zcash-hackworks/zcash-test-vectors/blob/master/orchard_key_components.py>.
//! These are the **Zcash-canonical reference values** the Zcash
//! audit pipeline uses to validate Orchard key derivations.
//!
//! Vendored (rather than imported via `orchard = "0.12"` dev-dep)
//! because orchard 0.12 transitively requires `core2 = "^0.3"`
//! which is yanked on crates.io. The vectors themselves are pure
//! data (test inputs + expected derivations); no code is copied.
//!
//! [`orchard` 0.12.0 crate]: https://github.com/zcash/orchard
//!
//! **License:** the orchard crate is MIT OR Apache-2.0; this
//! coincync-swap crate is MIT. Compatible. Original copyright +
//! license retained per the upstream LICENSE-MIT / LICENSE-APACHE.
//!
//! Used by `tests/zcash_conformance.rs` to assert byte-for-byte
//! conformance of every key-component derivation we ship in
//! `crates/orchard-side/src/spend_key.rs` against this Zcash-
//! canonical oracle. A byte-mismatch is a consensus-incompatibility
//! bug.

#![allow(dead_code)] // some fields not yet checked by every test
