...
A floor of thirty percent of every transaction fee is burned -- removed from circulation forever, captured by no one. This rate rises during network congestion and may never fall below the protocol's stated minimums. Combined with the asymptotic emission curve, this makes CYNC deflationary at sustained fee volume high enough that burned fees exceed the tail emission (>~2 CYNC/block). Fees not burned are paid to miners as proof-of-work reward; no fraction of any fee may ever be redirected to a developer fund, foundation, validator pool, or any other party.
