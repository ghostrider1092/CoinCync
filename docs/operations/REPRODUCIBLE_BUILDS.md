...
| First repro-verified release (Linux) | ✅ shipped — `release.yml` build-linux job invokes `bash ./scripts/build-in-docker.sh`. Every published Linux binary tarball from tag v1.0.10-testnet onward is byte-identical to what a community member produces with the same command at the same commit. Verify step compares tarball SHA256SUMS to the Docker `out/SHA256SUMS` in-workflow so a future refactor accidentally reverting to `target/release/` fails CI. |
| First repro-verified release (macOS + Windows) | ⏳ still uses `cargo build` on native runners — GitHub Actions has no Debian-Bookworm-equivalent reproducible-container ecosystem for those platforms today. Cross-arch (x86_64 vs arm64) also not guaranteed identical. |
